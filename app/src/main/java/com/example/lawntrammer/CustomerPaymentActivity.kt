...
class CustomerPaymentActivity : AppCompatActivity() {
    ...
    // Data structure for payment items
    ...
    private fun createPaymentCard(item: PaymentRecord): CardView {
        val card = CardView(this).apply {
            ...
            radius = 32f
            setCardBackgroundColor(if (isDarkMode) Color.parseColor("#1B2133") else Color.WHITE)
        ...
        if (item.status == "Paid") {
            ...
            actionSide.addView(TextView(this).apply {
                text = "Unpaid"
                setTextColor(Color.parseColor("#EF4444")) // Red for Unpaid
                textSize = 10f
                setPadding(0, 0, 0, 8)
            })
            actionSide.addView(MaterialButton(this, null, com.google.android.material.R.attr.materialButtonStyle).apply {
                text = "Pay Now"
                cornerRadius = 40
                textSize = 10f
